...
function interiorTabla(){
    let base=10, baseColor=0, baseCalidad=0, baseColorCalidad=0;
    let costePagina=0;
    let html="";
...
function validarFormularioRegistro(event) {
    ...
    if (username.length <= 3 || username.length >= 15 || !isNaN(username.charAt(0)) || !esAlfanumerico(username)) {
        mostrarError("usu", "El nombre de usuario debe comenzar con una letra y tener entre 3 y 15 caracteres.");
        hayErrores = true;
    }

    // Validación de contraseña
    const tieneMayuscula = [...password].some(char => char === char.toUpperCase() && isNaN(char));
    const tieneMinuscula = [...password].some(char => char === char.toLowerCase() && isNaN(char));
    const tieneNumero = [...password].some(char => !isNaN(char));
    
    if (password.length <= 6 || password.length >= 15 || !tieneMayuscula || !tieneMinuscula || !tieneNumero) {
        mostrarError("pwd", "La contraseña debe tener entre 6 y 15 caracteres, incluir una letra mayúscula, una minúscula y un número.");
        hayErrores = true;
    }
    ...
    if (!email.includes('@') || !email.includes('.') || email.indexOf('@') === 0 || email.indexOf('.') === email.length - 1 || email.indexOf('@') > email.lastIndexOf('.')) {
    ...
    if (!fechaValida) {
        mostrarError("fecha_nacimiento", "Debe tener al menos 18 años y la fecha debe estar en formato válido (YYYY-MM-DD).");
        hayErrores = true;
    }
...
function esAlfanumerico(texto) {
    for (let i = 0; i < texto.length; i++) {
        ...
        if (!((char >= 'a' && char <= 'z') || (char >= 'A' && char <= 'Z') || !isNaN(char))) {
            return false;
        }
    }
    return true;
